...
if __name__ == "__main__":
    ...
    parser.add_argument("--doccano_file", default="/data/1_qunosen/project/key_pharse/uie/data/json/demo.jsonl", type=str)
    parser.add_argument("--save_dir", default="/data/1_qunosen/project/key_pharse/uie/data/text", type=str, help="The path of data that you wanna save.")
    parser.add_argument("--negative_ratio", default=5, type=int, help="应用于提取任务，负样本数 = 正样本数 * negative_ratio")
    parser.add_argument("--splits", default=[0.8, 0.1, 0.1], type=float, nargs="*", help=" 依次为训练集(train)、验证集(dev)、测试集(test)")
    parser.add_argument("--task_type", choices=['ext', 'cls'], default="ext", type=str, help="任务类型：提取(ext)、分类(cls)")
    parser.add_argument("--options", default=["正向", "负向"], type=str, nargs="+", help="用于分类")
    ...
    parser.add_argument("--is_shuffle", default=True, type=bool, help="是否打乱")
    parser.add_argument("--seed", type=int, default=1000, help="Random seed for initialization")
    parser.add_argument("--separator", type=str, default='####', help=" 实体标签和分类标签的分割器")
